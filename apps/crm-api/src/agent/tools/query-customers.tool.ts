...
export function createQueryCustomersTool(segmentsService: SegmentsService) {
  const querySchema = z.object({
    filters: z
      .object({
        status: z.string().optional().describe('Customer status: active, churned, or at_risk'),
        preferredChannel: z.string().optional().describe('Channel: email, whatsapp, sms, or rcs'),
        minOrders: z.number().optional().describe('Minimum total orders'),
        maxOrders: z.number().optional().describe('Maximum total orders'),
        minSpent: z.number().optional().describe('Minimum total spent'),
        maxSpent: z.number().optional().describe('Maximum total spent'),
        daysSinceLastOrderGte: z.number().optional().describe('Days since last order >= this value'),
        daysSinceLastOrderLte: z.number().optional().describe('Days since last order <= this value'),
      })
      .optional()
      .describe('Filters to apply when querying customers'),
    limit: z.number().optional().default(10).describe('Max number of sample customers to return'),
  });
